...
public class Umlet {
  ...
  private static void printUsage() {
		JOptionPane.showMessageDialog(null,
				"USAGE: -action=convert -format=(jpg|pdf|svg|eps) -filename=inputfile.uxf",
				"USAGE",
				JOptionPane.INFORMATION_MESSAGE);
  }
  
  
  public static void doConvert(String fileName, String format) {
	...
	DiagramHandler handler = new DiagramHandler(file);
  	...
  	if(format!=null && format.equals("jpg")) handler.getFileHandler().doSaveAsJPG(fileNameMain+".jpg");
  	else if(format!=null && format.equals("pdf")) handler.getFileHandler().doSaveAsPdf(fileNameMain+".pdf");
  	else if(format!=null && format.equals("svg")) handler.getFileHandler().doSaveAsSvg(fileNameMain+".svg");
  	else if(format!=null && format.equals("eps")) handler.getFileHandler().doSaveAsEps(fileNameMain+".eps");
  }
	...
	if(args.length!=0) {
  		...
  		if(action==null && format==null && filename!=null) {
  			...
  			if(action.equals("convert")) {
  				umlet.gui = new StandaloneGUI(umlet);
  	  		    doConvert(filename, format);
  			} else printUsage();
  		} else 
  			printUsage();
