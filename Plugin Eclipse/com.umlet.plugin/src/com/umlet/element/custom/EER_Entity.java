...
public class EER_Entity extends Entity {
	...
	public StickingPolygon getStickingBorder() { //LME: define the polygon on which relations stick on
		...
		p.addLine(new Point(this.getWidth()-1,0), new Point(this.getWidth()-1,ySave-1));
		if(!hasAttributes) p.addLine(new Point(this.getWidth()-1,ySave-1), new Point(0,ySave-1));
		p.addLine(new Point(0,ySave-1), new Point(0,0));
		return p;
	}
} 
