...
public class StandaloneGUI extends UmletGUI {
	...
	private JMenuItem menu_file_save_as_jpg;
	...
	public void setDiagramChanged(DiagramHandler diagram, boolean changed) {
		...
		if(index != -1) {
			this.diagramtabs.setTitleAt(index,diagram.getName() + change_string); 	
		}
	...
	protected void init() {
	    ...
	    menu_file_save_as_jpg = new JMenuItem("Save as JPG...");
	    menu_file_save_as_jpg.addActionListener(mlistener);
	    ...
	    menu_file.add(menu_file_save_as_jpg);
		...
		menu_file_save_as_jpg.setEnabled(false);
	...
	public void diagramSelected(DiagramHandler handler) {
		if(handler == null)
		{
			...
			menu_file_save_as_jpg.setEnabled(true);
